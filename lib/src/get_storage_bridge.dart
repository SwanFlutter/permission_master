...
class GetStorageBridge {
  static const MethodChannel channel = MethodChannel('permission_master');
  ...
  Future<void> write<T>(String key, T value) async {
    try {
      if (value is Map<String, dynamic>) {
        await channel.invokeMethod('storage_write', {
          'key': key,
          'value': jsonEncode(value), // Serialize HashMap to JSON string
        });
      } else {
        await channel.invokeMethod('storage_write', {
          'key': key,
  ...
  Future<T> read<T>(String key, T defaultValue) async {
    try {
      final result = await channel.invokeMethod('storage_read', {
        'key': key,
  ...
  Future<bool> contains(String key) async {
    try {
      return await channel.invokeMethod('storage_contains', {'key': key});
    } on PlatformException catch (e) {
  ...
  Future<void> remove(String key) async {
    try {
      await channel.invokeMethod('storage_remove', {'key': key});
    } on PlatformException catch (e) {
  ...
  Future<void> clear() async {
    try {
      await channel.invokeMethod('storage_clear');
    } on PlatformException catch (e) {
