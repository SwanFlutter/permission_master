...
class _PermissionDemoPageState extends State<PermissionDemoPage> {
  ...
  void initState() {
    super.initState();
    debugPrint('PermissionDemoPage: initializing state');

    WidgetsBinding.instance.addPostFrameCallback((_) {
      debugPrint('Setting PermissionMaster context');
      PermissionMaster.setContext(context);
      debugPrint('Checking initial permissions...');
  ...
  Future<void> _requestPermission(
    ...
    try {
      ...
      if (attempts >= 2 &&
        ...
        debugPrint('Current permission status for $permission: $status');
        ...
        if (status != PermissionStatus.granted) {
          ...
          if (openSettings == true) {
            debugPrint('User chose to open settings for $permission');
            try {
              await _permissionMaster.openAppSettings();
              debugPrint('App settings should be open now');
      ...
      debugPrint('Requesting permission for $title');
      final result = await requestFunc();
      debugPrint('Permission result for $title: $result');
      _updatePermissionStatus(title, permission, result);
    } catch (e) {
      debugPrint('Error requesting $title permission: $e');
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
